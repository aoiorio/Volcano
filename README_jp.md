...
└── 📁lib
    └── 📁core
        └── errors.dart // エラーのタイプを書くため (e.g. BackEndError)
    └── 📁domain // 基盤となるabstractクラスを定義するため
        └── 📁entity
        └── 📁repository
        └── 📁usecase
    └── 📁gen // fluttergenで生成したsvgやimageを保管するため (自動生成)
    └── 📁infrastructure // domainで定義したクラスを具体化するため
        └── 📁datasource // Flutter側でAPIのURLを叩いてデータを取得するため
            └── 📁auth
            └── 📁todo
            └── 📁type_color_code
            └── 📁user
        └── 📁dto // 取得するデータの型を定義するため
        └── 📁model // ユーザーがインプットする値の型を定義するため
            └── 📁auth
            └── 📁todo
            └── 📁user
        └── 📁repository // datasourceのクラスを使って、domainのrepositoryを実装するため
    └── 📁presentation // UIを作るため
        └── 📁component
            └── 📁global
            └── 📁modal
            └── 📁sign_in
        ...
        └── 📁theme
            └── theme.dart
    └── 📁usecase // repositoryで具体化したメソッドを実行するusecaseを実装するため
    └── main.dart
```
